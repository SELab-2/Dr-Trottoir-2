...
class TestWasteView(APITestCase):
    ...
    def test_list(self):
        r = self.client.get('/api/waste/', follow=True)
        self.assertEqual(r.status_code, status.HTTP_200_OK)
        r = self.client.get('/api/waste?date=2000-01-01', follow=True)
        self.assertEqual(r.status_code, status.HTTP_200_OK)
        r = self.client.get('/api/waste?date=vandaag', follow=True)
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        r = self.client.get('/api/waste?start=2000-01-01', follow=True)
        self.assertEqual(r.status_code, status.HTTP_200_OK)
        r = self.client.get('/api/waste?start=vandaag', follow=True)
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        r = self.client.get('/api/waste?randomparam=1', follow=True)
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        r = self.client.get('/api/waste?date=2000-01-01&randomparam=1', follow=True)
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)

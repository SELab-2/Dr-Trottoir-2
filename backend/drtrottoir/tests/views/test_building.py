...
class TestBuildingView(APITestCase):
    ...
    def test_waste(self):
        ...
        # Start and End param
        r = self.client.get(f'/api/building/{self.building.pk}/waste?start={data_now["date"]}&end={data_now["date"]}', follow=True)
        self.assertEqual(r.status_code, status.HTTP_200_OK)
        self.assertEqual(len(r.data), 1)
        self.assertEqual(r.data[0]["waste_type"], data_now["waste_type"])
        # Wrong parameter use
        ...
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        r = self.client.get(f'/api/building/{self.building.pk}/waste?date={data_now["date"]}&start={data_past["date"]}', follow=True)
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        # Wrong building id

...
export default function Employees(){
    ...
    const rowOptions = [["Edit", editRow], ["Delete", deleteRow], ["Mail", mailRow]]
    ...
    return (
        <>
          ...
          <main className={`h-screen p-12 flex-col justify-between`} style={{ backgroundColor: BG_LIGHT_SECONDARY }}>
          ...
          <PrimaryCard>
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {users.map((user, index) => (
                  <tr key={index} onContextMenu={(event) => handleRightClick(event, user)}>
            ...
            {contextMenu.show && <ContextMenu x={contextMenu.x} y={contextMenu.y} closeContextMenu={closeContextMenu} options={rowOptions}></ContextMenu>}

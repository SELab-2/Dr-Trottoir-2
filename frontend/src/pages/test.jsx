...
export default function TestPage() {
  ...
  const postTour = async () => {
    const response = await TourService.post({
      name: "tour",
      buildings: [],
      region: "http://localhost:8000/api/region/1/",
    });
    setResponse(JSON.stringify(response, null, 2));
  };
  ...
  const postBuildingInTour = async () => {
    const response = await BuildingInTourService.post({
      tour: "http://localhost:8000/api/tour/1/",
      building: "http://localhost:8000/api/building/1/",
      order_index: 0,
    });
    setResponse(JSON.stringify(response, null, 2));
  };
    ...
    <>
      <div className={"w-1/2"}>
        ...
        <div>
          ...
          <PrimaryButton onClick={patchIdBuildingInTour} className={"w-1/3"}>
            <p>PATCH ID Building in Tour</p>
          </PrimaryButton>
          <PrimaryButton onClick={patchURLBuildingInTour} className={"w-1/3"}>
            <p>PATCH URL TOBuilding in TourUR</p>
          </PrimaryButton>
        </div>

        <div>
          <PrimaryButton onClick={deleteIdBuildingInTour} className={"w-1/3"}>
            <p>DELETE ID Building in Tour</p>
          </PrimaryButton>
          <PrimaryButton onClick={deleteURLBuildingInTour} className={"w-1/3"}>
